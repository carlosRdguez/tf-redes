...
void ChatClient::login(const QString &userName)
{
    if (m_clientSocket->state() == QAbstractSocket::ConnectedState) { // if the client is connected
        ...
        clientStream.setVersion(QDataStream::Qt_4_5);
        // Create the JSON we want to send
        QJsonObject message;
        message[QStringLiteral("tipo")] = QStringLiteral("login");
        message[QStringLiteral("usuario")] = userName;
        // send the JSON using QDataStream
        clientStream << QJsonDocument(message).toJson(QJsonDocument::Compact);
    }
}

void ChatClient::setKeyWord(const QString &keyWord)
{
    this->keyWord = keyWord.toLocal8Bit();
}
...
void ChatClient::jsonReceived(const QJsonObject &docObj)
{
    ...
    if (typeVal.toString().compare(QLatin1String("login"), Qt::CaseInsensitive) == 0) { //It's a login message
        ...
        const QJsonValue resultVal = docObj.value(QLatin1String("exito"));
        ...
        const QJsonValue reasonVal = docObj.value(QLatin1String("razon"));
        emit loginError(reasonVal.toString());
    } else if (typeVal.toString().compare(QLatin1String("mensaje"), Qt::CaseInsensitive) == 0) { //It's a chat message
        ...
        const QJsonValue senderVal = docObj.value(QLatin1String("remitente"));
        ...
        QString encryptMessage = textVal.toString();
        QByteArray pub, priv;
        QRSAEncryption e(QRSAEncryption::Rsa::RSA_128);
        e.generatePairKey(pub, priv, keyWord); // or other rsa size
    ...
    } else if (typeVal.toString().compare(QLatin1String("usuarioDesconectado"), Qt::CaseInsensitive) == 0) { // A user left the chat
        qDebug() << "usuario desconectado" ;
        // we extract the username of the new user
        const QJsonValue usernameVal = docObj.value(QLatin1String("usuario"));
        if (usernameVal.isNull() || !usernameVal.isString())
            return; // the username was invalid so we ignore
